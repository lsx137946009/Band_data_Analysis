...
class LowPass(BaseFilter):
    ...
    """    
    def __init__(self, cutoff, order=2, freq=None):
        self.cutoff = cutoff
        self.freq = freq
        self.order = order
        
    def fit(self, data):
        super(ButterLow, self).fit()
        if data.frequency:
            self.freq = data.frequency
        nyq = self.freq / 2
        wn = cutoff / nyq
        '''
        the critical frequencies. For a Butterworth filter, this is the point at 
        which the gain drops to 1/sqrt(2) that of the passband (the “-3 dB point”).
...
class Notch(BaseFilter)
    ...
    >>> b, a = butter_highpass(cutoff = 4.5, sample_rate = 12.5, order = 5)
    """    
    def __init__(self, cutoff, Q=0.005, freq=None):
        self.cutoff = cutoff
        self.freq = freq
        self.Q = Q
